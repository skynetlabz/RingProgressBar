...
public class RingProgressBar extends View {
    ...
    //to show % symbol
    public static boolean show_symbol = true;
    ...
    private void drawTextContent(Canvas canvas) {
        ...
        //获取文字的宽度 用于绘制文本内容
        float textWidth;
        //width if % symbol is used or not
        if (show_symbol)
            textWidth = paint.measureText(percent + "%");
        else
            textWidth = paint.measureText(percent + "");
        //绘制文本 会根据设置的是否显示文本的属性&是否是Stroke的样式进行判断
        if (textIsShow && percent != 0 && show_symbol && style == STROKE) {
            canvas.drawText(percent + "%", centre - textWidth / 2, centre + textSize / 2, paint);
        } else if (textIsShow && percent != 0 && style == FILL) {
            canvas.drawText(percent + "%", centre - textWidth / 2, centre + textSize / 2, paint);
        } else if (textIsShow && percent != 0 && !show_symbol && style == STROKE) {
            canvas.drawText(percent + "", centre - textWidth / 2, centre + textSize / 2, paint);
        }
    }
